...
begin
  ...
  loop
    ...
    if v_best.dist_days <= 7
      ...
      v_tier := 'attach';
      v_detail := format('%s at %.1f days%s', v_best.job_number, v_best.dist_days,
                         case when v_second.id is null then ', only job'
                              else format(', next candidate %s at %.1f days', v_second.job_number, v_second.dist_days) end);
      if not p_dry_run then
        ...
        values
          (v_company, v_best.id, v_kind, f.name,
           ...
           format('Texted in by %s on %s. Matched to %s by tf_unfiled_media_match (m340): nearest job at %.1f days, no rival within 7 days.',
                  ...
                  v_best.job_number, v_best.dist_days),
           v_msg.created_at, false);
      end if;
      v_attached := v_attached + 1;
    else
      v_tier := 'close_call';
      v_detail := format('two candidates inside the margin: %s at %.1f days, %s at %.1f days',
                         v_best.job_number, v_best.dist_days,
                         coalesce(v_second.job_number,'(none)'), coalesce(v_second.dist_days, 0));
      v_close := v_close + 1;
    end if;
