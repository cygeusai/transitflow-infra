...
## Working practices (this folder)
- [`PHONE_BUNDLE_HANDOFF.md`](./PHONE_BUNDLE_HANDOFF.md) — recovering commits from a
  Claude Code session that cannot push, by moving them as a git bundle. The
  producing and receiving command sets, the `HEAD`-versus-`refs/heads/<branch>`
  trap that breaks the receiving fetch, and the phone-only variant.
