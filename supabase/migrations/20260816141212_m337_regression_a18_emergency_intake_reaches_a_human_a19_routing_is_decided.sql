-- m337_regression_a18_emergency_intake_reaches_a_human_a19_routing_is_decided
--
-- m335 and m336 fixed the answering-service emergency path. This migration is
-- what stops it regressing, because the failure mode was not a crash. Nothing
-- errored. Seven flooded basements were recorded, classified, routed and
-- reported as handled, and no human was told. A defect that reports success is
-- the kind you only catch with an assertion.
--
-- A18 walks the whole live path with a synthetic payload, in the same order
-- production does: classify the body, resolve the audience, resolve the
-- severity tier's channels, render the message. It writes nothing. It fails if
-- any link breaks, INCLUDING the render quietly falling back to the ban-list
-- generic, which would deliver a message that tells the reader nothing.
--
-- PROVEN RED BEFORE SHIPPING GREEN. Run against the key quo-webhook actually
-- used before m335, the same predicate returns:
--   event_key tech_update, routed_and_actionable 0, tier_carries_slack 0,
--   render '(bell) Kept in the Hub / Nothing was posted to Slack for this one.'
--   => A18 false.
-- Run against the live classifier it returns:
--   event_key fnol_emergency, routed_and_actionable 1, tier_carries_slack 1,
--   render carries the callback number, the street and 'Dispatch now'
--   => A18 true.
--
-- A19 is the smaller of the two and the one that ages best. tf_notification_drain
-- already fails an unrouted key loud to #information-technology, so an omission
-- is never silent. A19 is the difference between a destination somebody chose
-- and a destination the fallback happened to get right. It is green today at
-- zero unrouted keys, and m336 made it green by deciding inbound_sms_not_logged
-- rather than leaving it to the fallback.
--
-- The suite goes from 17 assertions to 19. Nothing existing is modified.
